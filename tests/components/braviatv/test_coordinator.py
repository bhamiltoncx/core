...
async def test_sources_populated_when_tv_off(
    ...
    Before the fix, source_list stayed empty when HA started while the TV
    was already off, causing the tile card media-player-source feature to
    report 'Not compatible'.
    """
    mock_bravia_client.get_power_status = AsyncMock(return_value="standby")
    mock_bravia_client.get_content_list_all = AsyncMock(return_value=[])
...
async def test_sources_not_populated_on_connection_error(
    ...
    mock_bravia_client.get_power_status = AsyncMock(return_value="standby")
    mock_bravia_client.get_external_status = AsyncMock(
        side_effect=BraviaConnectionError()
    )
...
async def test_sources_repopulated_when_tv_turns_on(
    ...
    mock_bravia_client.get_power_status = AsyncMock(return_value="active")
    mock_bravia_client.get_external_status = AsyncMock(
        return_value=[
    ...
    mock_bravia_client.get_content_list_all = AsyncMock(return_value=MOCK_CHANNELS)
